...
export const countOverlaps = (grid: number[][][]): number => {
  return grid.reduce((count: number, row: number[][]): number => {
    return count + row.reduce((rowCount: number, cell: number[]) => {
      if (cell.length > 1) {
        return rowCount + 1;
      }
      return rowCount;
...
export const findUniqueId = (grid: number[][][], maxId: number): number => {
  const overlapped = Array.apply(undefined, {length: maxId}).map((v: any) => v || false);
  grid.forEach((row) => {
    row.forEach((cell) => {
      if (cell.length > 1) {
        cell.forEach(v => {overlapped[v - 1] = true; });
      }
    });
  });
