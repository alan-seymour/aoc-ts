...
export const incrementState = (
  grid: SeatGrid,
  surr: SurroundingAlgo,
  allowedNearby: number,
): SeatGrid =>
  grid.map((line, i) =>
    line.map((seat, j) => {
      if (seat === '.') return '.';
      ...
      if (
        (seat === '#' && surroundingCount < allowedNearby) ||
        (seat === 'L' && surroundingCount === 0)
      )
        return '#';
      return 'L';
    }),
...
const countOccupied = (grid: SeatGrid): number =>
  grid.reduce((sum, line) => line.filter(s => s === '#').length + sum, 0);
