...
export const parseInput = (input: string): number[][] => {
  const grid = splitLines(input).map(l => l.split('').map(d => parseInt(d, 10)));
...
const getLocationCost = (grid: number[][], { x, y }: Coord, tile: boolean): number | null => {
  if (!tile) {
  ...
  const scaled = value + Math.floor(y / grid.length) + Math.floor(x / grid.length);
