...
export const parseInput = (input: string): number[][] => {
  const map = splitLines(input).map(l => l.split('').map(c => parseInt(c, 10)));
...
export class Puzzle202109 extends PuzzleDay {
  part1() {
    ...
    const totalRisk = lowPoints.reduce((s, c) => s + map[c.x][c.y], 0) + lowPoints.length;
  ...
  part2() {
    ...
    const basinSizes = lowPoints.map(l => findBasinSize(map, l)).sort((a, b) => b - a);
