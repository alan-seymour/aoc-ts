...
const isPair = (v: string | Pair): v is Pair => {
  return typeof v === 'object' && typeof v.index === 'bigint' && typeof v.value === 'bigint';
};

const chineseRemainder = (buses: string[]): BigInt => {
  const numericalBuses = buses.map((v, i) => Number(v) ? { value: BigInt(v), index: BigInt(i) } : 'x')
    .filter(isPair);
...
export class Puzzle202013 extends PuzzleDay {
  part1() {
    ...
    const first = distanceFromEarliest.shift()!;
