...
export const runAplifiersWithPhases = (state: number[], phases: number[]): number => {
...
export const runAmplifierLoop = (initialState: number[], phases: number[]): number => {
  const amplifierA = new IntCodeComputer({
    state: initialState,
    input: [phases[0], 0],
  });
...
export class Puzzle201907 extends PuzzleDay {
  part1() {
    ...
    const outputs = trialPhases.map(phase => runAplifiersWithPhases(initialState, phase));
  ...
  part2() {
    ...
    const outputs = trialPhases.map(phase => runAmplifierLoop(initialState, phase));
