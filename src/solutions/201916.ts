...
export const parseInput = (input: string): number[] => input.split('').map(v => parseInt(v, 10));
...
const lazyFind = (offset: number, input: number[]): string => {
  ...
  const phasedDigits = input.slice(width * -1).map((d: number): number[] => new Array(101).fill(d));

  for (let d = width - 2; d >= 0; d--) {
    for (let phase = 0; phase < 100; phase++) {
      phasedDigits[d][phase + 1] = (phasedDigits[d][phase] + phasedDigits[d + 1][phase + 1]) % 10;
    }
  }
...
const getOffset = (input: string): number => parseInt(input.substr(0, 7), 10);
