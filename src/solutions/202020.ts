...
export const edgesMatch = (edgeOne: string[], edgeTwo: string[]): boolean =>
  isEqual(edgeOne, edgeTwo) || isEqual(edgeOne, edgeTwo.slice().reverse());
...
export const findMatchingLeftEdge = (edge: string[], tiles: Tile[]): [Tile, number] | undefined => {
  for (let i = 0; i < tiles.length; i++) {
    ...
    if (isEqual(tiles[i].edges.bottom.slice().reverse(), edge))
      return [flipTileVertically(rotateTileRight(tiles[i])), 4];

    if (isEqual(tiles[i].edges.left, edge)) return [tiles[i], 5];
    if (isEqual(tiles[i].edges.left.slice().reverse(), edge))
      return [flipTileVertically(tiles[i]), 6];

    if (isEqual(tiles[i].edges.right, edge)) return [flipTileHorizontally(tiles[i]), 7];
    if (isEqual(tiles[i].edges.right.slice().reverse(), edge))
      return [rotateTileLeft(rotateTileLeft(tiles[i])), 8];
  }
...
export const flipTileHorizontally = ({
  id,
  cells,
...
export const flipTileVertically = ({
  id,
  cells,
  edges: { top, bottom, left, right },
}: Tile): Tile => ({
...
export const rotateTileRight = (tile: Tile): Tile => flipTileHorizontally(transposeTile(tile));

export const rotateTileLeft = (tile: Tile): Tile => transposeTile(flipTileHorizontally(tile));
...
export const solveImage = (tiles: Tile[]): Tile[][] => {
  ...
  for (let j = 1; j < dimension; j++) {
    ...
    const [nextRight, idx] =
      findMatchingLeftEdge(image[0][j - 1].edges.right, availableTiles) ?? [];
  ...
  for (let i = 1; i < dimension; i++) {
    ...
    for (let j = 0; j < dimension; j++) {
      ...
      if (j === 0) {
        const [nextFirst, idx] =
          findMatchingLeftEdge(image[i - 1][0].edges.bottom, availableTiles) ?? [];
        ...
        const [nextRight, idx] =
          findMatchingLeftEdge(image[i][j - 1].edges.right, availableTiles) ?? [];
...
export const stitchGrid = (tiles: Tile[][]): string[][] =>
  tiles
    .map(row =>
      row[0].cells.map((col, i) =>
        row.reduce<string[]>((prev, current) => prev.concat(current.cells[i]), []),
      ),
    )
...
export const stitchCells = (cells: string[][][][]): string[][] =>
  cells
    .map(row =>
      row[0].map((col, i) => row.reduce<string[]>((prev, current) => prev.concat(current[i]), [])),
    )
    .reduce((prev, current) => prev.concat(current), []);
...
export const getEdgelessCells = ({ cells }: Tile): string[][] =>
  cells.map(line => line.slice(1, -1)).slice(1, -1);
...
export const findMonsters = (grid: string[][]): { grid: string[][]; monsterCount: number } => {
  ...
  for (let i = 0; i < newGrid.length - monsterShape.length; i++) {
    for (let j = 0; j < newGrid[0].length - monsterShape[0].length; j++) {
      const section = newGrid
        .map(line => line.slice(j, j + monsterShape[0].length))
        .slice(i, i + monsterShape.length);

      const hasMonster = checkSectionForMonster(section);

      if (hasMonster) {
        monsterCount++;
        newGrid = markMonster(newGrid, i, j);
      }
    }
  }

  return { grid: newGrid, monsterCount };
};

export const flipGridHorizontally = <T>(grid: T[][]): T[][] =>
...
export const rotateGridRight = <T>(grid: T[][]): T[][] => flipGridHorizontally(transposeGrid(grid));
