...
const part1valid = (passport: Passport): boolean =>
  requiredKeys.every(key => passport[key] !== undefined);
...
const keyValidation: { [key: string]: (val: string) => boolean } = {
  ...
  hcl: val => /^#[0-9a-fA-F]{6}$/.test(val),
  ecl: val => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].some(c => c === val),
  pid: val => /^\d{9}$/.test(val),
};

const part2valid = (passport: Passport): boolean =>
  requiredKeys.every(key => passport[key] !== undefined && keyValidation[key](passport[key]));
