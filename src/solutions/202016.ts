...
export const parseInput = (input: string): ParsedInput => {
  ...
  const yourTicket = splitLines(yourSection)[1]
    .split(',')
    .map(v => parseInt(v, 10));

  const otherTickets = splitLines(othersSection)
    .slice(1)
...
const invalidFields = (ticket: number[], rules: Rule[]) =>
  ticket.filter(
    field =>
      !rules.some(
        rule =>
...
const possibleFieldTitles = (values: number[], rules: Rule[]) =>
  rules.filter(rule =>
    values.every(
      field =>
        (field >= rule.range1Min && field <= rule.range1Max) ||
...
export class Puzzle202016 extends PuzzleDay {
  part1() {
    ...
    const invalidTotals = input.otherTickets.reduce(
      (sum, t) => invalidFields(t, input.rules).reduce((tot, c) => tot + c, 0) + sum,
      0,
    );
  ...
  part2() {
    ...
    const potentiallyValid = input.otherTickets.filter(
      t => invalidFields(t, input.rules).length === 0,
    );
